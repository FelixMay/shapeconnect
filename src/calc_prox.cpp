...
// [[Rcpp::export]]
DataFrame calc_prox(IntegerMatrix map, double cell_size, double alpha){
...
// [[Rcpp::export]]
DataFrame calc_prox2(IntegerMatrix map, double cell_size, double alpha, int n_frag){

   int nrows = map.nrow();
   int ncols = map.ncol();

   IntegerVector frag_ncells(n_frag);
   NumericMatrix connect_mat(n_frag, n_frag);
   ...
   IntegerVector frag_id(n_frag);
   NumericVector frag_area_ha(n_frag);
   NumericVector frag_connect(n_frag);

   for (int i = 0 ; i < n_frag; ++i){
      frag_id[i] = i;
      frag_area_ha[i] = frag_ncells[i] * cell_size * cell_size/10000;
      for (int j = 0 ; j < n_frag; ++j){
         frag_connect[i] += connect_mat(i,j);
      }
   }
